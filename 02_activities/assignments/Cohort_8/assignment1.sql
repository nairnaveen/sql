...
SELECT* FROM customer
ORDER BY customer_last_name, customer_first_name ASC -- a->z
...
WHERE product_id IN (4,9); --list only product ids 4 and 9
...
COUNT (DISTINCT booth_number) AS num_of_times_booth_rented
FROM vendor, vendor_booth_assignments
GROUP By vendor.vendor_id, market_date;
