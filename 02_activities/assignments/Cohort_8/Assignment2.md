...
```
Your answer... The store can use the Slowly Changing Dimensions architecture. The SCD Type 1 architecture will allow the store to overwrite the changes in Customer address, while the SCD Type 2 architecture will preserve the history of all changes as SCD Type 2 adds a new row to the table whenever an address is updated.
```
